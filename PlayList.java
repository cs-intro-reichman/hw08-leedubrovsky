...
class PlayList {
    ...
    public int indexOf(String title) {
        ...
        for (int i = 0; i < this.size; i++){
            if (this.getTrack(i) != null){
                if (getTrack(i).getTitle().toLowerCase().equals(title.toLowerCase())){ ///takes from specific track the title and checks if its equal to the title given else return -1 (for safety both titles converted to lower cases to check if there is n absolot diffrence) 
                return i;
                }
            }
        }
     ...
     *  (t5, t3, t1), then just after add(1,t4) the list becomes (t5, t4, t3, t1). //// add t4 in the index 1
    ...
    //// An elegant and terribly inefficient implementation.
     public void add(PlayList other) {
        //// replace this comment with your code
        int sum = other.getSize() + this.getSize();
        if (sum <= getMaxSize())
        while (other.size != 0 ){
